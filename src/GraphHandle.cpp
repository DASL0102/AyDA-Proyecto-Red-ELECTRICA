...
std::tuple<
     ...
     auto node_insert2 = grafoRed.search_node([=](auto node)
    {
     ...
     ArcoInfo arc_final = {min_distance, 2.0, 0.0};
...
void GraphHandle::best_connectivity(
    ...
    grafoRed2.for_each_node([&](auto* node) {
        if (node != nodo_g2) {
            ...
            ArcoInfo arc_inset = {dist, 0.0, 0.0};
    ...
    if (closest_node) {
        ...
        if (casa_real && destino_real) {
            ...
            double dist = std::sqrt(dx*dx + dy*dy);
            ArcoInfo arc_final = {dist, 0.0, 0.0};
...
void GraphHandle::connect_graph(GenericGraph<NodoRed>& grafoRed, std::shared_ptr<NodoRed> a, std::shared_ptr<NodoRed> b) {
     ...
     Designar::DigraphNode<std::shared_ptr<NodoRed>, ArcoInfo, Designar::EmptyClass>* nodo_a , nodo_b;
...
std::vector<TrackNode> GraphHandle::lookingNodes2( GenericGraph<NodoRed>& grafoRed,GenericGraph<NodoRed>& grafoAlone, std::vector<TrackNode> unconnetedArcs, std::string last_s_id) {
    std::cout<<"here\n";
    int contadordev = 0;
            ...
            if(node->get_info()->getTipo() != "Estacion" and node->get_info()->getTipo() != "SubEstacion"){
                ...
                auto n1 = grafoRed.search_node([&](auto node_) {
                    return node_->get_info()->getname() == node1_insert->get_info()->getname()
                           ...
                           && node_->get_info()->getX() == node1_insert->get_info()->getX()
                           && node_->get_info()->getY() == node1_insert->get_info()->getY();
                ...
                auto n2 = grafoRed.search_node([&](auto node_) {
                   return node_->get_info()->getname() == node2_insert->get_info()->getname()
                           ...
                           && node_->get_info()->getX() == node2_insert->get_info()->getX()
                           && node_->get_info()->getY() == node2_insert->get_info()->getY();
                });
    ...
    // std::cout << "=== Contenido de unconetedArcs ===" << std::endl;
        ...
        for (const auto& arc : unconnetedArcs) {
            ...
            auto source_ga = grafoAlone.search_node([&source](auto node) {

                return node->get_info()->getname() == source->get_info()->getname();

            });

            
            auto tgt_ga = grafoAlone.search_node([&target](auto node) {
                //auto node_info = node->get_info();
                
                //if(node_info->getname() == target->get_info()->getname()){
                    return node->get_info()->getname() == target->get_info()->getname();
                //}
            });
        ...
        //grafoAlone.insert_arc(source_ga, tgt_ga, std::get<2>(arc)); 
            ...
            if(casa_nodo_cast->getConsumption() <= subEstacion_nodo_cast->getCurrentCharge()){
                ...
                if(sb_ptr != nullptr){
                    ...
                    if(last_s_id != sb_ptr->getId()){
                        unconetedArcs_aux = lookingNodes2(grafoRed, grafo, unconetedArcs_aux, sb_ptr->getId());
                    }else{
